    ...
    plotRsquared = FALSE)
{
  ...
  # Create a new df with the same dimensions as corvals and fill with P values
  pvals <- corvals
  ...
  # Filter out non-significant modules based on the p-value threshold
  sig_modules <- colnames(pvals)[apply(pvals, 2, function(x) any(x < 0.05))]
  
  # Keep only significant correlation values and corresponding p-values for these modules
  corvals <- corvals[, sig_modules]
  pvals <- pvals[, sig_modules]
  
  # Define the colour scheme/palette
  cols <- grDevices::colorRampPalette(col)
  
  # Create a new df with the same dimensions as corvals
  ...
  # Return the list of significant module names (returns a list of module names)
  # return(sig_modules)
}